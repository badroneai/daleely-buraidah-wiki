  ...
  return [
    {
      ...
      note: '????? ?????? ?????? ??????.',
      children: [
        { key: 'cafes', title: '?????????', status: 'active', note: '?????? ???????? ??????.' },
        { key: 'restaurants', title: '???????', status: 'planned', note: '???? ??????? ??? ???? ?????? ???.' },
        { key: 'bakeries', title: '??????? ?????????', status: 'planned', note: '???? ??????? ???? ??????? ??????.' }
      ]
    }
  ];
}
...
function renderBlueprintPage() {
  return `
    ...
    <div class="grid cards-3 section">
      <div class="card"><h3>?????</h3><p class="note">????? ????? ?????? ??????? ?? ???? ???? ?????. ?? ??? ???? ??? <code>master.json</code>.</p></div>
      <div class="card"><h3>???????</h3><p class="note">??????? ????? OpenClaw ???? ??????? ???????? ????? ??? ?? ????? ??? ?????? ???????.</p></div>
      <div class="card"><h3>??????</h3><p class="note">?????? ??? ???????: ???? ???? ? ?????? ????? ? ?????? ? ????? ???????.</p></div>
    </div>
  `;
}
    ...
    <div class="section grid cards-2">
      ${tree.map(group => `
        <div class="card">
          ...
          <ul class="list-clean">
            ${group.children.map(child => `<li><strong>${child.key === 'cafes' ? `<a href="#/sector/cafes">${esc(child.title)}</a>` : esc(child.title)}</strong><div class="note">${esc(child.note)} ? ${child.status === 'active' ? '???' : '????'}</div></li>`).join('')}
          </ul>
        </div>
      `).join('')}
    </div>
...
function renderSectorPage(sectorKey = 'cafes') {
  if (sectorKey !== 'cafes') {
    return `<div class="hero"><h3>???? ??? ???????</h3><p>??? ?????? ???? ???????? ???? ?? ???? ?????? ???.</p></div>`;
  }
  ...
  return `
    ...
    <div class="grid cards-4">
      <div class="card"><div class="metric">${items.length}</div><div class="metric-sub">?????? ???????</div></div>
      <div class="card"><div class="metric">${uniq(items.map(r=>r.district)).length}</div><div class="metric-sub">???????/???????</div></div>
      <div class="card"><div class="metric">${items.filter(r=>r._norm.specialty_coffee==='yes').length}</div><div class="metric-sub">???? ?????</div></div>
      <div class="card"><div class="metric">${items.filter(r=>r._norm.work_friendly==='yes').length}</div><div class="metric-sub">????? ?????</div></div>
    </div>
...
function renderPipelinePage() {
  return `
    ...
    <div class="grid cards-4 section">
      <div class="card"><h3>1) ??????</h3><p class="note">??? ???? ???? ?? ???? ?????.</p></div>
      <div class="card"><h3>2) Intake</h3><p class="note">New Cafe / Import Draft / Queue for Agent Import.</p></div>
      <div class="card"><h3>3) Agent Processing</h3><p class="note">OpenClaw ????? ????? ?????? result ?proposed patch.</p></div>
      <div class="card"><h3>4) ?????? ???????</h3><p class="note">?????? ????? ?? ????? patch ??? ?????? ??????? ???.</p></div>
    </div>
  `;
}
...
function renderGovernancePage() {
  return `
    ...
    <div class="grid cards-3 section">
      <div class="card"><h3>???????? ?????????</h3><p class="note">?????? ??????? ?? <code>master.json</code>? ??? ????? ?????? ?? ???????.</p></div>
      <div class="card"><h3>??????? ???????</h3><p class="note">???? ????????? ?? ???? best-effort? ????????? ??????? ?????? ??? ???????.</p></div>
      <div class="card"><h3>???? ?????????</h3><p class="note">???? ???? import queue ?? processor ?? import alpha naming ???? ??? ??????.</p></div>
    </div>
  `;
}
